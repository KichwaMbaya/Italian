...
public class Contattaci extends AppCompatActivity {
    ...
    private void contacusmessage() {
        ...
        if (csendtxt.isEmpty()) {
            sendtxt.setError("Digita qualcosa.");
            return;
        }
        if (!Conectivity.isConnectingToInternet(Contattaci.this)) {
            Toast.makeText(Contattaci.this, "Per favore controlla la tua connessione Internet", Toast.LENGTH_SHORT).show();
            return;
        }
        progressDialog = new ProgressDialog(Contattaci.this);
        progressDialog.setMessage("Invio...");
